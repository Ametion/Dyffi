...
func (g *Engine) SetLogger(logger logging.ILogger) {
	g.logger = logger
}
			...
			if g.matchRoute(route, requestParts) {
				...
				if ctx := g.processRoute(route, sw, r, requestParts); ctx != nil {
					...
					g.logger.LogInfo(
						...
						logging.Field{Key: "status", Value: sw.status},
...
func (g *Engine) UseBroker(brokerConfig dyffiBroker.BrokerConfig) {
	...
	if err != nil {
		g.logger.LogError("Error creating broker", logging.Field{"error", err.Error()})
		return
	}
...
// Run starts the web server and emits a clean, ordered startup log.
func (g *Engine) Run(addr string) error {
	// 1) Header
	g.logger.LogInfo("Dyffi Engine starting with the following routes:")

	// 2) Routes
	if len(g.routes) == 0 && len(g.graphqlSchemas) == 0 {
		g.logger.LogInfo("No routes registered!")
	} else {
		for _, route := range g.routes {
			...
			// one log line per route
			// new: a single “registered route” message, fields hold the details
			g.logger.LogInfo(
				"registered route",
				logging.Field{"method", route.method},
		...
		if len(g.graphqlSchemas) > 0 {
			g.logger.LogInfo("GraphQL Models Registered:")
			for modelName := range g.graphqlSchemas {
				g.logger.LogInfo(
					modelName,
					logging.Field{"model", modelName},
				)
			}
		}
	}
	...
	// 4) Listening
	g.logger.LogInfo("Listening on " + addr)
